"""
01_preprocess.py
Loads raw_merged.csv, derives all composite variables, and saves clinical_data.csv.
"""
import pandas as pd
...
# Fill missing with median (avoids inplace)
...
# ---- Composite variables ----
# Note: suffix is the raw sheet suffix after merge (_PRE_RAW or _POST_RAW for pre-sheet cols,
# and the actual POST columns keep their original names)
# We need to map the correct columns.
# After merge, PRE sheet columns get suffix _PRE_RAW, POST sheet columns keep originals
# OR get suffix _POST_RAW if they matched PRE column names.

# Simplify: find columns by their content patterns
...
log.log(f"VAS PRE missing after conversion: {df['vas_pre'].isna().sum()}")
log.log(f"VAS POST missing after conversion: {df['vas_post'].isna().sum()}")
